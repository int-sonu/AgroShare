...
import { 
  MapPin, Phone, Calendar, ShieldCheck, Info, 
  Fuel, Gauge, Lock, User, Truck, Tag, EyeOff,
  Maximize2, LockIcon, Mail, UserCheck
} from 'lucide-react';
...
export default function MachineDetailPage() {
  ...
  if (!machine) {
    return <div className="text-center py-10 px-4">
      ...
      <Button className="mt-4 bg-green-600 h-9 rounded-lg text-sm" onClick={() => window.history.back()}>Go Back</Button>
    </div>;
  }
              ...
              {images.map((img: any, idx: number) => (
                <button
                  ...
                  className={`relative w-14 h-14 md:w-16 md:h-16 rounded-md overflow-hidden border-2 transition-all shrink-0 bg-white ${
                    activeImage === idx ? 'border-green-600 shadow-sm' : 'border-gray-100 hover:border-gray-200'
                  }`}
                >
                  <Image
                    src={getImageUrl(img)}
          ...
          <div className="lg:col-span-6 flex flex-col space-y-3 lg:pl-1">
            <div className="space-y-0">
              <div className="flex items-center gap-2 mb-1">
                <span className="text-[10px] font-bold text-green-600 uppercase tracking-widest">{machine.brandModel || "Universal"}</span>
                <span className="w-1 h-1 rounded-full bg-gray-300"></span>
                <span className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">{machine.yearOfManufacture || "N/A"}</span>
              </div>
              ...
              <div className="flex items-center gap-1 text-gray-500 text-[11px] font-medium pt-0.5">
                ...
                {!isLoggedIn && <span className="text-[9px] text-gray-400 font-bold ml-1 flex items-center gap-0.5"><LockIcon className="w-2 h-2" /> Village hidden</span>}
              </div>
            </div>
            ...
            <div className="bg-gradient-to-r from-green-50 to-emerald-50 p-2.5 rounded-xl border border-green-100 flex justify-between items-center transition-all">
              <div>
                <p className="text-[9px] font-bold text-green-700/60 uppercase tracking-widest mb-0">Daily Price</p>
                <div className="flex items-baseline gap-1">
                  <span className="text-2xl font-black text-green-700">₹{machine.pricing?.rentalPrice?.toLocaleString()}</span>
                  <span className="text-[11px] font-bold text-green-600/80">/ {machine.pricing?.priceType}</span>
                </div>
              </div>
              <div className="h-8 w-px bg-green-200/50"></div>
              {machine.pricing?.minimumRentalDuration && (
                <div className="text-right">
                  <p className="text-[9px] font-bold text-green-700/60 uppercase tracking-widest mb-0">Min Booking</p>
                  <p className="text-[11px] font-black text-green-700">{machine.pricing.minimumRentalDuration} {machine.pricing.priceType.split(' ')[1] || 'Unit'}</p>
                </div>
              )}
            </div>

            <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
              <DetailItem icon={<ShieldCheck className="w-3 h-3"/>} label="Condition" value={machine.condition || "Excellent"} />
              <DetailItem icon={<Gauge className="w-3 h-3"/>} label="Power" value={`${machine.machineCapacityHP || '45'} HP`} />
              <DetailItem icon={<Fuel className="w-3 h-3"/>} label="Fuel" value={machine.fuelType || "Diesel"} />
              <DetailItem 
                icon={<Lock className="w-3 h-3"/>} 
                label="Deposit" 
                value={isLoggedIn ? (machine.pricing?.securityDeposit ? `₹${machine.pricing.securityDeposit}` : "None") : "Login to view"} 
                isLocked={!isLoggedIn}
              />
              <DetailItem 
                icon={<Truck className="w-3 h-3"/>} 
                label="Logistics" 
                value={machine.transport?.transportAvailable ? (isLoggedIn ? `₹${machine.transport.transportCost}` : "Available") : "N/A"} 
                isLocked={!isLoggedIn && machine.transport?.transportAvailable}
              />
              <DetailItem icon={<Calendar className="w-3 h-3"/>} label="MFG Year" value={machine.yearOfManufacture || "N/A"} />
            </div>
              ...
              {isLoggedIn ? (
                <div className="p-2.5 rounded-xl border border-gray-100 bg-gray-50/40 hover:bg-white transition-colors">
                  <div className="flex items-center gap-1.5 mb-1">
                    ...
                    <span className="text-[10px] font-bold text-gray-800 uppercase tracking-wider">Operator Info</span>
                  </div>
                  {machine.operator?.operatorIncluded ? (
                    <div className="space-y-0.5">
                      <p className="text-[11px] text-gray-700 font-bold">{machine.operator.operatorName || 'Assigned Driver'}</p>
                      <p className="text-[10px] text-gray-600 font-medium flex items-center gap-1">
                        <Phone className="w-2.5 h-2.5 text-green-600" /> {machine.operator.operatorPhone || 'N/A'}
                      </p>
                    </div>
                  ) : (
                    <p className="text-[10px] text-gray-400 italic font-medium pt-0.5">Self-operate only</p>
                  )}
                </div>
              ) : (
                <div className="p-2.5 rounded-xl border border-dashed border-gray-200 bg-gray-50/20 flex flex-col items-center justify-center py-2 text-center">
                  ...
                  <p className="text-[9px] text-gray-500 font-bold uppercase tracking-tight">Operator Info Locked</p>
                </div>
              )}
              ...
              {isLoggedIn ? (
                <div className="p-2.5 rounded-xl border border-gray-100 bg-gray-50/40 hover:bg-white transition-colors">
                  <div className="flex items-center gap-1.5 mb-1">
                    ...
                    <span className="text-[10px] font-bold text-gray-800 uppercase tracking-wider">Seller Contact</span>
                  </div>
                  <div className="space-y-0.5">
                    <p className="text-[11px] text-gray-700 font-bold">{machine.seller?.name || 'Authorized Seller'}</p>
                    <div className="flex flex-col gap-0.5">
                      <p className="text-[10px] text-gray-600 font-medium flex items-center gap-1">
                        <Phone className="w-2.5 h-2.5 text-green-600" /> {machine.seller?.phone || 'N/A'}
                      </p>
                      <p className="text-[10px] text-gray-600 font-medium flex items-center gap-1">
                        <Mail className="w-2.5 h-2.5 text-green-600" /> {machine.seller?.email || 'N/A'}
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-2.5 rounded-xl border border-dashed border-gray-200 bg-gray-50/20 flex flex-col items-center justify-center py-2 text-center">
                  <LockIcon className="w-3.5 h-3.5 text-gray-300 mb-1" />
                  <Button onClick={() => router.push('/auth/login')} variant="ghost" className="h-4 p-0 text-[10px] text-green-600 font-black hover:bg-transparent uppercase tracking-tight">
            ...
            <div className="space-y-1">
              ...
              <p className="text-gray-600 text-[11px] leading-snug line-clamp-2 hover:line-clamp-none transition-all duration-300">
                {machine.description || "Top-tier machinery for all your professional agricultural needs. Well-maintained and recently serviced for peak performance."}
              </p>
            </div>
            ...
            <div className="flex gap-2.5 pt-1 mt-auto">
              ...
              >
                {isLoggedIn ? <><Phone className="w-3.5 h-3.5 mr-1.5" /> Contact Seller</> : "Login to View Contact"}
              </Button>
              <Button 
                variant="outline" 
                onClick={() => isLoggedIn ? null : router.push('/auth/login')}
...
function DetailItem({ icon, label, value, isLocked = false }: DetailItemProps) {
  return (
    <div className={`p-2 rounded-lg border border-gray-50 bg-gray-50/30 flex flex-col gap-0 group transition-all shadow-[inset_0_1px_2px_rgba(0,0,0,0.01)] ${!isLocked && 'hover:bg-white hover:border-green-100'}`}>
      <div className={`flex items-center gap-1 text-gray-400 ${!isLocked && 'group-hover:text-green-500'} transition-colors`}>
        {icon}
        <span className="text-[8px] font-extrabold uppercase tracking-tight">{label}</span>
      </div>
      <div className="flex items-center gap-1">
        <p className={`text-[10px] font-black truncate text-gray-800`}>
          {value}
        </p>
